...
import {
  acquireMultipleSeatLocks,
  releaseMultipleSeatLocks,
} from "../redis/seatLock";
...
export async function reserveSeats(
  ...
  if (seats.length !== seatIds.length) {
    throw new AppError(
      "One or more seat IDs are invalid or do not belong to this event",
      404,
    );
  }
  ...
  if (bookedSeat) {
    throw new AppError(`Seat ${bookedSeat.seatNumber} is already booked`, 409);
  }

  const lockResult = await acquireMultipleSeatLocks(eventId, seatIds, userId);

  if (!lockResult.success) {
    throw new AppError(`Seat is currently reserved by another user`, 409);
  }
